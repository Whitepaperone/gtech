...
def main():
    ...
    try:
        ...
        process_name_col = start_col - 1
        product_col = start_col - 2
        customer_code_col = start_col - 3
        model_col = start_col - 4
        ...
        for i in range(start_row, len(df)):
            ...
            process_name = row_full[process_name_col]
            ...
            processed_rows.append([model, customer_code, product, process_name] + adjusted)
        ...
        result_df.columns = ["모델", "고객사 품번", "품번", "공정 인쇄"] + date_row

        result_df.iloc[:, 4:] = result_df.iloc[:, 4:].apply(pd.to_numeric, errors='coerce').fillna(0)

        agg_dict = {"모델": "first", "고객사 품번": lambda x: x.dropna().iloc[0] if not x.dropna().empty else "", "공정 인쇄": "first"}


        for col in result_df.columns[4:]:
            agg_dict[col] = "sum"
        ...
        result_df = result_df[(result_df.iloc[:, 4:] != 0).any(axis=1)]

        output_path = file_path.replace(".xlsx", "_작업지시.xlsx")
